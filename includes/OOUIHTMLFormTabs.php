...
class OOUIHTMLFormTabs extends OOUIHTMLForm {
	...
	protected function wrapFieldSetSection( $legend, $section, $attributes, $isRoot ) {
		...
		$layout->addClasses( [ 'ext-renamewiki-fieldset-wrapper' ] );
	...
	public function getBody() {
		...
		foreach ( $this->mFieldTree as $key => $val ) {
			if ( !is_array( $val ) ) {
				LoggerFactory::getInstance( 'RenameWiki' )->debug(
			...
			$tabPanels[] = new TabPanelLayout( "mw-section-$key", [
				...
				'content' => new FieldsetLayout( [
					'classes' => [ 'ext-renamewiki-section-fieldset' ],
					'id' => "mw-section-$key",
					'label' => $label,
					'items' => [
						new Widget( [
		...
		$form = new PanelLayout( [
			'framed' => true,
			'expanded' => false,
			'classes' => [ 'ext-renamewiki-tabs-wrapper' ],
			'content' => $indexLayout,
		] );
