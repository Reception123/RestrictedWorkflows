( function () {
	$( () => {
		...
		const tabs = OO.ui.infuse( $( '.ext-renamewiki-tabs' ) );
		tabs.$element.addClass( 'ext-renamewiki-tabs-infused' );

		function enhancePanel( panel ) {
			const $infuse = $( panel.$element ).find( '.ext-renamewiki-infuse' );
		 ...
		 */
		function switchRenameWikiTab( name, noHash ) {
			if ( noHash ) {
		...
		function detectHash() {
			...
			if ( hash.match( /^#mw-section-[\w-]+$/ ) ) {
				mw.storage.session.remove( 'renamewiki-prevTab' );
				switchRenameWikiTab( hash.slice( 1 ) );
			} else if ( hash.match( /^#mw-[\w-]+$/ ) ) {
				matchedElement = document.getElementById( hash.slice( 1 ) );
				$parentSection = $( matchedElement ).closest( '.ext-renamewiki-section-fieldset' );
				if ( $parentSection.length ) {
					mw.storage.session.remove( 'renamewiki-prevTab' );
					// Switch to proper tab and scroll to selected item.
					switchRenameWikiTab( $parentSection.attr( 'id' ), true );
		...
		$( window ).on( 'hashchange', () => {
			...
			if ( hash.match( /^#mw-[\w-]+/ ) ) {
				...
				switchRenameWikiTab( $( '[id*=mw-section-]' ).attr( 'id' ), true );
			}
		} )
		...
		const previousTab = mw.storage.session.get( 'renamewiki-prevTab' );
		if ( previousTab ) {
			switchRenameWikiTab( previousTab, true );
			// Deleting the key, the tab states should be reset until we press Save
			mw.storage.session.remove( 'renamewiki-prevTab' );
		}

		$( '#renamewiki-form' ).on( 'submit', () => {
			const value = tabs.getCurrentTabPanelName();
			mw.storage.session.set( 'renamewiki-prevTab', value );
		} );
	} );
}() );
